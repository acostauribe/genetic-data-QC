...
It is important to know how the chromosomes numbers are encoded in your vcf, e.g. they can be '1' or 'chr1'. You have to use the same string for the command to work
...
At multiple steps in of this process we will like to check quality metrics, such as missing data. \
We have provided a shell script `calculate_quality_metrics.sh` to generate the seven following files:

I. General statistics (file.vchk)
...
3. VQSR quality (file.vqsr) 

```{bash autosome-preQC-get-stats, eval=FALSE, cache=FALSE, include=FALSE}
bash calculate_qual_metrics.sh $PREFIX.autosomes | tee calculate_qual_metrics.$PREFIX.autosomes.txt
```
...
# Create a new environment variable. This variable will hold everything related to the data before QC
pre_QC = new.env()

plot_imiss =  function(ref_base, pathToFile, pre) { # environment variable, VCF file path, TRUE/FALSE
