...
[5. Calculate relatedness](## 5. Calculate relatedness)
[6. Final clean up](## 6. Remove variants & Individuals and with missingness ≥5%)
...
knitr::opts_chunk$set(root.dir = "/home/acostauribe/ReDLat/Training", tidy=TRUE)
setwd("/home/acostauribe//ReDLat/Training")

# Set up R variables:
PREFIX='redlat_unimputed_array'

# Set up environment variables and path to software:
Sys.setenv(PREFIX='redlat_unimputed_array')
...
```
**NOTICE** that individuals missing more than 5% of the data get removed in this step "X people removed due to missing genotype data (--mind).'

Plot missingess per variant and per individual
```{r missingess-plots}
# A. Load data into R
## Before QC
...
Calculate heterozygosity per sample before QC `${PREFIX}` and after the --mind --geno filtering `${PREFIX}.mind.geno`
This step requires that the .bim file has variant IDs in the second column. If no variants have been assigned, you could do a preliminary step using [--set-missing-var-ids](https://www.cog-genomics.org/plink/1.9/data#set_missing_var_ids)
...
```
This step uses a *sliding window* approach to identify variants in linkage disequilibrium. There are many options to modify the behavior or this approach in [plink's docummentation](https://www.cog-genomics.org/plink/1.9/ld#indep)
...
abline(h = (heterozygosity_low_limit_qc), col="hotpink")
abline(h = (heterozygosity_high_limit_qc), col="hotpink")
abline(h = (mean(het_qc$F)), col="lavender") 
```
...
If the values are 0 or -9, you can assign 'sex value' in plink:
`sex_info.txt` has the sex information in three columns. Each row is FID, IID, sex(1=male, 2=female, 0=no data)
FID, IID should match the `.fam`
```{bash assign-sex}
...
Make sure you are using the correct genome alignment for this. ReDLat files are aligned to hg38.
```{bash split-PAR}
$PLINK --bfile ${PREFIX}.mind.geno.sex --split-x hg38 --make-bed --out ${PREFIX}.mind.geno.sex.split-x
```
...
```{bash check-sex-X}
## a. Retain X y Y
$PLINK --bfile ${PREFIX}.mind.geno.sex.split-x --chr 23-24 --make-bed --out ${PREFIX}.mind.geno.sex.split-x.XY

## b. Calculate r2 between the variants in a given window
## This step requires that the .bim file has variant IDs in the second column
$PLINK --bfile ${PREFIX}.mind.geno.sex.split-x.XY --indep-pairwise 50 5 0.2
## This command will generate two files: plink.prune.in y plink.prune.out

## c. Retain unlinked variants
$PLINK --bfile ${PREFIX}.mind.geno.sex.split-x.XY --extract plink.prune.in --make-bed --out ${PREFIX}.mind.geno.sex.split-x.XY.LD

## d. Calculate heterozygosity F statistic of X chromosome
$PLINK --bfile ${PREFIX}.mind.geno.sex.split-x.XY.LD  --check-sex 0.3 0.7 --out ${PREFIX}.mind.geno.sex.split-x.XY.LD.chrX
```
...
sex_X = read.delim(file = paste0(PREFIX,".mind.geno.sex.split-x.XY.LD.chrX.sexcheck"), 
                    header = TRUE, 
                    sep = '')
...
```{bash check-sex-Y}
$PLINK --bfile ${PREFIX}.mind.geno.sex.split-x.XY --check-sex y-only --out ${PREFIX}.mind.geno.sex.split-x.XY.LD.chrY
```
...
sex_Y = read.delim(file = paste0(PREFIX,".mind.geno.sex.split-x.XY.LD.chrY.sexcheck"), 
                    header = TRUE, 
                    sep = '')
...
ggplot(sex_Y, 
    ...
    geom_quasirandom(alpha = 0.7,
                   size = 1.5) + 
    labs(title = "Chromosome Y count in samples - Raw dataset",
         x = "Disclosed sex",
         y = "Y chromosome count") +
  theme_minimal() +
  theme(legend.position = "none")
ggsave("Y chromosome count - raw data.png", width = 8, height = 5) 
```

## 4. Sumarize all sample metrics
...
After these analyses you will have to generate a file with the individuals that fail
each line represents one sample [e.g. samples_het_sex_exclude.txt]
FID    I-D\
PLO0001 PLO0001\
PBS001 PBS001\
...
Each line represents two samples that have heterozygote concordance rate > 80%. 
These samples need to be carefully addressed to determine if they are *known* biological duplicates (e.g. same patient sequenced twice),
or if they are supposed to be *different* samples. 
For the *known* biological duplicates, you should retain the genome with the best metrics.
If the genomes are from *different* samples, both genomes should be removed (you cannot tell which one does the genome belong to)

**YOU** need to generate a text file with the genomes that have to be excluded e.g. `duplicates.txt`. Each line has to be `FID` `ID`
...
```
The output of this command produces two files: \
`${PREFIX}.kin` Relatedness in reported relationships 
`${PREFIX}.kin0` Inferred relatedness in 'unrelated' individuals \
Each row above provides information for one pair of individuals. See [king documentation](https://www.kingrelatedness.com/manual.shtml#RELATED)

if you get `FATAL ERROR - Please correct problems with pedigree structure` it usually means that an idividual who is, for example, listed as male but shows up as the mother of another individual.

**YOU** need to manually analyze these two files and generate a table including the samples that have problematic relatedness and have to be excluded e.g. problematic_relatedness.txt . Each line has to be `FID` `ID`.
...
```{bash mendel-errors}
$PLINK --bfile ${PREFIX}.mind.geno.sex.het.dup.fid.pid.rel --set-me-missing --mendel-duos --make-bed --out ${PREFIX}.mind.geno.sex.het.dup.fid.pid.rel.me
```
