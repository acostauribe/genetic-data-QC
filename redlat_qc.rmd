...
if (data_type == "ARRAY"){
  ...
  sample_statistics = rename(sample_statistics, initital_missingness = F_MISS, Initial_n_sites = N_GENO)
} else if (exists("idepth")){ 
  sample_statistics = rename(idepth, IID = INDV, Initial_n_sites = N_SITES, mean_initial_depth = MEAN_DEPTH)
  ...
  sample_statistics$initital_missingness = imiss$F_MISS[match(sample_statistics$IID, imiss$INDV)]
} else {
  sample_statistics = subset(imiss, select = c(INDV, F_MISS, N_DATA))
  sample_statistics = rename(imiss, IID = INDV, initital_missingness = F_MISS, Initial_n_sites = N_DATA)
}
# Save as a file
