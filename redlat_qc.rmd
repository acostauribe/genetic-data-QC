...
knitr::opts_chunk$set(root.dir = "path/to/your/directory",
                      dev = "png",
                      dpi = 300,
                      echo = FALSE,
                      cache = TRUE)
...
Sys.setenv(king='/path/to/your/king')
...
if (file.exists("sample_data.txt")) {
    ...
    print(".fam file will be annotated with: sample_data.txt")
  } else { 
...
echo "No genotype quality control was performed"
fi
```
...
# Plot heterozygosity per sample
#par(mar = c(3, 3, 3, 8), xpd = TRUE) # create margin around plot
...
missing_data = setdiff(fam$V2, sample_data$IID)
fam_IID = fam[,2]
sample_IID = sample_data$IID
```
...
het_sex_dup_ids = unique(het_sex_dup)

if(data_type=='EXOME' || data_type=='GENOME'){
write.table(select(het_sex_dup_ids, IID),
            "het_sex_dup.irem", 
            col.names = FALSE, 
            row.names = FALSE, 
            quote = FALSE)
  } else {
  write.table(het_sex_dup_ids,
            "het_sex_dup.irem",
            col.names = FALSE,
            row.names = FALSE, 
            quote = FALSE)
...
$plink --bfile ${prefix} --merge-x --make-bed --out ${prefix}.merged-x
...
if [ $tidy_up == 'TRUE' ]; then
  ...
  mkdir Sex_check
  mv ${prefix}.chrX.* ./Sex_check
  mv ${prefix}.chrY.* ./Sex_check
...
```{bash import-plink}
# If you have a vcf file, import it into plink 
if [ $data_type == 'GENOME' ] || [ $data_type == 'EXOME' ] && [ $check_relatedness == 'TRUE' ]; then
...
echo "No relatedness check was performed"
fi
```
...
if [ $tidy_up == 'TRUE' ]; then
  ...
  if [ $data_type == 'EXOME' ] || [ $data_type == 'GENOME' ]; then 
  mv *.mind-2.ikeep ./Missingness
  mv *.mind-2.irem ./Missingness
  mv *.mind-2.imiss ./Missingness
  fi
