...
knitr::opts_chunk$set(root.dir = "/home/acostauribe/Genetic-Sequencing_raw-data/HudsonAlpha_J.Nicholas.Cochran/redlat/redlat_QC",
                      dev = "png",
                      dpi = 300,
                      echo = FALSE,
                      cache = TRUE)
...
## Give the paths to plink, king,vcftools, bcftools
...
# Import-sample-data}
## This command expects the file to be named "sample_data.txt". If your file has a different name, change it in the line below. 
if (file.exists("sample_data.txt")) {
    sample_data = read.delim("sample_data.txt", header = TRUE, sep = '\t')
    print(".fam will be annotated with: sample_data.txt")
  } else { 
...
## 'TRUE' Includes only sites with DP/GQ (over all included individuals) greater than or equal to the given value. 'FALSE' Skips this step
genotype_filter=FALSE
# Define the values you want to use as threshold. We recommend a DP=10 and GQ=20
DP=10
GQ=20

# Do you want to keep only the variants with PASS in the VQSR filtering?
## This can only be done if you start with a VCF file that was filtered trough "Variant Quality Score Recalibration"
## 'TRUE' Removes all sites with a FILTER flag other than PASS in the vcf. 'FALSE' skips this step
vqsr_PASS=FALSE
...
## 'TRUE' uses KING to perform relatedness check. 'FALSE' skips this step
check_relatedness=TRUE

# Do you want to create directories and to organize your data as you go?
## TRUE generates directories and organizes your files as you run the pipeline. 'FALSE' leaves all output in the working directory
tidy_up=TRUE
```

After setting up your environment and customizing your pipeline, run this chunk to make r values into system environment variables
...
} else if (file.exists(paste0(prefix,".preqc.idepth"))){ 
...
if(data_type=='EXOME' || data_type=='GENOME'){
  ...
  print(ldepth_mean)
  ### Plot depth per site
  ldepth_box = boxplot(MEAN_DEPTH ~ CHROM,
                       data=ldepth,
                       xlab="Mean Depth",
                       ylab="Sites",
...
if(data_type=='EXOME' || data_type=='GENOME'){
statistical_measures = bind_rows(imiss_F,
                                 idepth_mean,
                                 lmiss_F,
                                 ldepth_mean)
} else {
...
write.table(statistical_measures,
...
statistical_measures = bind_rows(statistical_measures,
...
if(sum(fam$V5) == 0) {
  fam$V5 = sample_data$SEX[match(fam$V2, sample_data$IID, nomatch = 0, all.x = TRUE)]
  write.table(fam,
              paste0(prefix,".fam"), 
              col.names = FALSE, 
              row.names = FALSE, 
...
het_sex_dup = bind_rows(het_outlier_id,
                        sex_fail_id,
                        duplicates_id)
